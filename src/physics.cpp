...
bool plcollide(dynent* d, dynent* o, float& headspace, float& hi,
...
bool collide(dynent* d, bool spawn, float drop, float rise)
{
    ...
    float minfloor = (d->monsterstate && !spawn && d->health > 100)
        ...
        : -1000.0f; // big monsters are afraid of heights, unless angry :)
    ...
    for (int x = x1; x <= x2; x++)
        for (int y = y1; y <= y2; y++) // collide with map
        {
            ...
            case CORNER: {
                int bx = x, by = y, bs = 1;
                if (x == x1 && y == y1 && cornertest(0, x, y, -1, -1, bx, by, bs) && fx1 - bx + fy1 - by <= bs || x == x2 && y == y1 && cornertest(0, x, y, 1, -1, bx, by, bs) && fx2 - bx >= fy1 - by || x == x1 && y == y2 && cornertest(0, x, y, -1, 1, bx, by, bs) && fx1 - bx <= fy2 - by || x == x2 && y == y2 && cornertest(0, x, y, 1, 1, bx, by, bs) && fx2 - bx + fy2 - by >= bs)
                    return false;
                break;
            };
...
void moveplayer(dynent* pl, int moveres, bool local, int curtime)
{
    ...
    } else // apply velocity with collision
    {
        if (pl->onfloor || water) {
            ...
            } else if (pl->timeinair > 800) // if we land after long time must have
    ...
    if (pl->o.x < 0 || pl->o.x >= ssize || pl->o.y < 0 || pl->o.y > ssize) {
        ...
        pl->outsidemap = SOLID(s) || pl->o.z < s->floor - (s->type == FHF ? s->vdelta / 4 : 0) || pl->o.z > s->ceil + (s->type == CHF ? s->vdelta / 4 : 0);
    };
