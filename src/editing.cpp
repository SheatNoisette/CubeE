...
float sheight(sqr* s, sqr* t,
    float z) // finds out z height when cursor points at wall
{
...
{
    EDITSEL;
    if (type == CORNER && (sel.xs != sel.ys || sel.xs == 3 || sel.xs > 4 && sel.xs != 8 || sel.x & ~-sel.xs || sel.y & ~-sel.ys)) {
...
void arch(int sidedelta, int _a)
{
    ...
    if (sel.ys > MAXARCHVERT)
        sel.ys = MAXARCHVERT;
    loopselxy(s->vdelta = sel.xs > sel.ys ? (archverts[sel.xs - 1][x] + (y == 0 || y == sel.ys - 1 ? sidedelta : 0)) : (archverts[sel.ys - 1][y] + (x == 0 || x == sel.xs - 1 ? sidedelta : 0)));
    remipmore(sel);
};
