...
{
    static const unsigned char pixels[] = { 255, 0, 255, 0, 0, 0,
        0, 0, 0, 255, 0, 255 };
...
void gl_init(int w, int h)
{
    ...
    if (exts && (strstr(exts, "GL_EXT_texture_env_combine") || strstr(exts, "GL_ARB_texture_env_combine")))
        hasoverbright = true;
    else {
        if (!exts)
            conoutf(
                "WARNING: GL_EXTENSIONS unavailable, disabling overbright lighting");
        else
            conoutf(
...
bool installtex(int tnum, char* texname, int& xs, int& ys, bool clamp)
{
    ...
    while (scaledxs > glmaxtexsize || scaledys > glmaxtexsize) {
        scaledxs /= 2;
        scaledys /= 2;
    };
    ...
    if (gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, scaledxs, scaledys, GL_RGBA, GL_UNSIGNED_BYTE, scaledimg))
...
{
    if (hasoverbright)
        glTexEnvf(GL_TEXTURE_ENV, GL_RGB_SCALE, amount);
};
...
VARFP(gamma, 30, 100, 300, {
    conoutf("WARNING: gamma correction not supported in SDL3");
});
